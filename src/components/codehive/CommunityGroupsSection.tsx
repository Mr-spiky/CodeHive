...
function GroupCard({ group }: { group: (typeof COMMUNITY_GROUPS)[number] }) {
  return (
    <GoldCard topAccent className="p-6">
      {/* Icon box */}
      <div className="w-12 h-12 rounded-xl bg-[#C9A227]/10 border border-[#C9A227]/25 flex items-center justify-center text-2xl mb-4 group-hover:bg-[#C9A227]/20 group-hover:border-[#C9A227]/50 transition-all duration-300">
        {group.icon}
      </div>

      <h3 className="font-sora text-lg font-bold text-white mb-1">{group.title}</h3>
      ...
      <a
        ...
        className="inline-flex items-center gap-2 text-sm font-semibold text-[#C9A227] hover:gap-3 transition-all duration-300 group-hover:text-[#E5C84A]"
      >
        Join Now <ArrowIcon />
      </a>
